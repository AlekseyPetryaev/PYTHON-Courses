"""
Задача 49.Планеты вращаются вокруг звезд по эллиптическим орбитам. Назовем самой далекой планетой 
ту, орбита которой имеет самую большую
площадь. 
...
длины полуосей эллипса орбиты самой далекой планеты. Каждая орбита представляет из себя кортеж из
пары чисел - полуосей ее эллипса. Площадь эллипса вычисляется по формуле S = pi*a*b,
где a и b - длины полуосей эллипса. При решении задачи используйте списочные выражения.
Подсказка: проще всего будет найти эллипс в два шага: сначала вычислить самую большую площадь 
эллипса, а затем найти и сам эллипс, имеющий такую площадь. Гарантируется, что самая далекая  планета ровно одна
...
# Вариаант 2 находим минимальное 
...
# Ответ
# 9 78 43 37
# (1, 3) # нашло менимальное 


# Вариаант 3 находим махсимальное 

orbits = [(1, 3), (2.5, 10), (7, 2), (6, 6), (4, 3)]
orbits_square = lambda orbits: list(int(3.14 * i[0] * i[1]) for i in orbits if i[0] !=  i[1])
print(*orbits_square(orbits))
find_farthest_orbit = lambda array: list(i for i in orbits if int(3.14 * i[0] * i[1]) == max(orbits_square(orbits)))      
print(*find_farthest_orbit(orbits))


# Ответ
# 9 78 43 37
# (2.5, 10)  # максимальное нашло
